...
`premise` 和 `diversity.situation` 必须对应同一个具体事件或静态视觉命题，而非
仅复述“协作”“雨夜”等题材标签。事件须明确当前动作、具体对象或问题；非叙事设计
则明确主体与空间关系，不强加冲突或剧情。人物、地点、时间范围及已经确定的外观
事实构成 Frame 的共同边界，不能让不同 Frame 各自发明无关任务来填补泛泛的 Theme。
...
Frame 写作规则要求静默检查手部占用、手臂可达性、承重与物件接触是否相容，
以及衣物结构和可见状态是否连续；一件连体衣物不能无依据地拆成可独立脱卸的两件。
表情和视线须回应当前动作，正文应写出可见证据，而非“物理正确”等自我认证。
这些是生成约束，不是本地语义判定器；schema、字符数或确定性去重通过，并不证明
自由文本没有事件漂移、人物复制或肢体矛盾，仍需要对真实输出进行质量抽查。
...
Story pipeline 不设置人物国籍或场景国家缺省值，也不要求每个 Theme premise
或最终 Frame 固定输出国籍、国家和“所在国家”字段。Story Description 或专属
authoring 明确指定的国籍与地点仍作为普通视觉事实保留；未指定时不补写。
年代、文化风格、姓名、语言、肤色、外貌和物质语汇不单独构成国籍或国家声明。
...
配方、模块和目录的 YAML 自然语言内容统一使用中文，包括描述、创作
...
两个阶段仍可分别使用不同模式。
Theme 的 `premise`、`style` 和 Frame 的字符范围另附一个写作建议目标：
取有效上下限的整数中点，例如 550–1050 字符建议写到约 800 字符。该目标只引导
模型分配具体内容，不新增校验阈值，也不改变重试条件；上下限内的任意长度仍可接受。
短标题、上下限相同的精确长度和空白分词范围不附加中点建议。不会在本地填字、
截断或追加无关环境细节来满足范围，也不声称一次实测就能校准所有模型的写作长度。
字符约束按每个 Frame 独立应用，而非按批次合计；包裹标签不计入正文长度。
