...
Theme 提供共同情境、人物关系和可见事件，不预先锁定每帧的表情、反应强度和
视线落点。Frame 先确定当前实际行为，再选择最适合该行为的表情，以动作目的、
用力程度、身体受力和注意对象为首要依据，人物关系与作品表现依据用于细化。
面部张力、嘴部状态和视线须适合当前行为的注意需求与身体负荷，不能仅凭动作名称
机械分配情绪，例如用力不必然等于愤怒。至少两项相互协调的面部状态和自然可达的
视线落点共同呈现这种反应，不用心理独白或虚构身世解释。
同一瞬间不能把同一张嘴写成既紧闭又张开，或让同一双眼同时凝视两个不同目标；
允许有情境依据的混合情绪与局部不对称，但不允许互相否定的面部状态。

行为适配优先于表情多样性与风格修饰。表演幅度须符合情境、景别和编译上下文中
已有的作品表现依据，不按导演署名套用统一神情，也不为远景可读性刻意夸张。
多人可以共享主要情绪，相近行为可以自然产生相似表情；不强迫人物做不同表情，
也不按性别、名单顺序或固定角色模板机械分配情绪。
同 Theme 的 Frame 对照当前批次及选择性重试中的 `accepted_frame_prose`，
先检查表情是否最适合当前行为，再根据动作、用力程度或关注对象的真实差异调整
反应。允许有行为依据的相似表情，不设置表情差异配额，不强制每帧更换情绪标签，
也不把平行静帧写成情绪逐步升级的连续情节。

这些要求同步进入逐人物动态消息，不改变内容等级文件、请求参数、输出格式或
验证开关，不新增模型调用。现有面部与视线诊断仍是人物名附近的关键词统计，
不判断情绪因果、表演自然度或图像效果；回归测试验证规则和重试消息中的约束传递，
不代表模型实际输出已经满足这些要求。
