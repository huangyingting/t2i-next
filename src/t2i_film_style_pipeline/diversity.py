    ...
    participant_contracts = [
        {
            ...
            "description_requirement": (
                ...
                "唯一且自然可达的视线落点，以及前后一致且相互兼容的肢体用途；"
                "先定当前行为，再选择最适合该行为的表情，以动作目的、"
                "用力程度、身体受力和注意对象为首要依据；"
                "情绪须有当前可见事件与人物关系的依据，面部状态相互协调，"
                "表演幅度符合当前情境与已有作品依据，"
                "动作适配优先于表情多样性与风格修饰；可共享主要情绪，"
                "行为相近可有相似表情，但不得无依据套用模板；"
                "后续摄影和表情描写不得改变已确定的位置、朝向与支点"
            ),
        }
        ...
        if len(characters) >= 3:
            contract: dict[str, object] = {
                ...
                "topology_safety_requirement": (
                    "逐人闭合支撑并保持肢体用途兼容；同一接触的双方须采用"
                    "一致的解剖侧、位置与相互对应的受力方向，距离和高度差"
                    "须在既定姿态下可达。保留指定人数、身份和内容等级要求，"
                    "先保证支撑与关节合理、接触可达，再协调摄影与自然遮挡，"
                    "最后追求姿态变化与装饰细节；不得把任何人物降级为观察者"
                ),
                "spatial_requirement": (
                    f"为 {len(characters)} 人自行选择互不矛盾且可读的空间位置，"
                    "使每个人的面孔、主要身体轮廓、支撑关系和参与动作可判断；"
                    "允许接触面自然遮挡，不要求全部支撑面毫无遮挡"
                ),
                "camera_requirement": (
                    f"按 {len(characters)} 人群像选择能够容纳全部人物并呈现支撑"
                    "关系和参与动作的景别与机位；遮挡不合适先调整机位，不得"
                    "为镜头强扭关节、挪开承重物或因为近景而漏掉任何人物"
                ),
            }
