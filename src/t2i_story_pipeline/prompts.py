...
def _content_level_instruction(request: StoryRequest) -> str:
    return (
        ...
        "compliant content beside a separate task. When the story explicitly "
        "defines the selected level as a presentation contract for a "
        "non-narrative design or pose board, follow its wardrobe, coverage, and "
        "pose-intensity rules instead of the narrative activity guidance above; "
        "do not invent a sexual act or missing partner."
    ...
    *,
    ...
    system = "\n".join(
        (
            ...
            "If the story explicitly requests a non-narrative character or "
            "design board, treat the design concept as the complete situation "
            "and do not invent an event, relationship conflict, decision, or "
            "outcome. "
            "If the story explicitly requests a campaign, editorial concept "
            "board, advertising storyboard, or high-concept anthology, treat "
            "its visual proposition, hero system, and conceptual leap as the "
            "complete event logic. In this mode, override the ordinary premise "
            "pattern: use the first sentence to state what familiar thing "
            "changes function or meaning and the physical logic that makes the "
            "surprise immediately legible; use the second sentence to name six "
            "distinct visual-stunt seeds separated by semicolons. These compact "
            "stunt seeds are required even though shot-level detail is otherwise "
            "deferred to Frames. Do not reduce the concept to a person "
            "encountering a large setting, generic tactile exploration, an "
            "intimate dialogue with material, or ordinary relationship tension. "
...
def frame_messages(
    ...
    system = "\n".join(
        (
            ...
            "When the story explicitly requires a multi-view character or "
            "design board, repeated views of the same person are references to "
            "one cast member, not additional people. Follow the board's exact "
            "view, continuity, coverage, and labeling contract without inventing "
            "narrative action between views.",
            "When the story explicitly requires a campaign, editorial concept "
            "board, advertising storyboard, or high-concept anthology, every "
            "region must deliver a distinct visual stunt that advances the same "
            "conceptual proposition. Realize all six stunt seeds recorded in the "
            "Theme premise instead of replacing them with generic poses or "
            "material studies. Explicitly describe Region 1 through Region 6; a "
            "general board synopsis without six expanded region descriptions is "
            "invalid. When the story requires a fixed multi-region board, every "
            "Narrative Frame must independently contain one complete board, "
            "regardless of frames_per_theme. Never distribute one board across "
            "multiple Frames, use one Frame per region, or continue a region in "
            "another Frame. A six-region board should use roughly 450 to 650 "
            "words, or equivalent detail in the requested language, so every "
            "region is independently renderable. "
            "Follow the story's own flexible panel dramaturgy. "
            "Do not default to several camera views of touching, "
            "leaning against, reclining on, walking through, or bathing in an "
            "enlarged material, and do not impose a fixed climax position that "
            "the story did not request.",
