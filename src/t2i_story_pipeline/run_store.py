...
class LocalStoryRunStore:
    ...
    def checkpoint_frame(
        ...
        try:
            directory = self._run_directory(run_id)
            if self._read_manifest(directory).status == StoryRunStatus.COMPLETED:
                raise StoryStorageError(f"Run {run_id} 已完成，不能写入 checkpoint")
            request = StoryRequest.model_validate_json(
                (directory / "request.json").read_text(encoding="utf-8")
            )
            ...
            checkpoint = directory / "frames" / theme_id / f"{frame.frame_id}.json"
    ...
    def _load_frames(
        ...
        frames: dict[str, NarrativeFrameSequence] = {}
        for theme_directory in sorted((directory / "frames").iterdir()):
            if not theme_directory.is_dir() or theme_directory.name not in theme_ids:
                raise StoryStorageError(
                    f"Frame checkpoint 目录无效或缺少 Theme：{theme_directory.name}"
                )
            accepted: list[NarrativeFrame] = []
            for path in sorted(theme_directory.iterdir()):
                if path.name.startswith(".") and path.name.endswith(".tmp"):
                    continue
                if (
                    not path.is_file()
                    or path.suffix != ".json"
                    or path.stem not in expected_frame_ids
                ):
                    raise StoryStorageError(f"Frame checkpoint 文件无效：{path.name}")
            ...
            if accepted:
                frames[theme_directory.name] = NarrativeFrameSequence(frames=accepted)
        return frames
