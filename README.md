...
上限，不要求至少一名女性，但不能同时为零。只传一项时，另一项采用自己的默认值。
不再随机抽取人数。男女数量保存在请求中，Theme 和 Frame 收到相同的精确要求；
同一运行的全部 Theme、Frame、重试和 resume 都沿用这两个值。
Profile 尽量完整提取可确认的原作成年人物，避免只给后续流程一对主角或固定截断名单；
原作人物不足时不允许为凑数量虚构身份。
所有 Frame 保持选定人物一致，不能复制一个身份来凑人数。此选择规则对现有所有内容
...
后续 Theme 批次恢复传入全部已接受 Theme 的完整标题、premise 和 style，
不再压缩成短摘要或只保留最近几个；resume 也从完整 checkpoint 重建上下文。每个
新 Theme 必须区别于全部历史和本批其他 Theme；仅改标题、措辞、人数或小道具
不能算新主题。程序只额外拦截规范化后 premise/style 完全重复的批内或历史主题，
进入已有有界重试；不增加模型评审阶段或相似度阈值。语义避重仍是写作要求，
不是本地完整证明；完整历史比短摘要消耗更多输入 token。每个
...
视线落点落实，不能用群体共同表情或抽象情绪标签代替逐人描写。情绪须有当前事件
与人物关系的依据；先定当前行为，再选择最适合它的表情，以动作目的、用力程度、
身体受力和注意对象为首要依据。面部状态相互协调，表演幅度符合情境和已有作品
依据，但行为适配优先于表情多样性与风格修饰，不能只凭导演署名推定统一表情。
允许人物共享主要情绪，行为相近也可自然产生相似表情；行为或注意需求变化时，
反应才相应变化。不设置表情差异配额，不强制更换情绪标签，也不制造逐帧发展的
情绪时间线。逐人物动态消息同步这些要求；
诊断仍只是关键词统计，不代表情绪或表演正确性验证。
为减少肢体错乱，Frame 按支撑面、躯干朝向、兼容的肢体用途、双方一致且可达的
接触链和衣物最终状态构造。允许单脚主要承重、另一脚轻触，也允许同一只手撑住
并抓紧同一处桌沿，但禁止同一肢体同时出现在不同位置。身体安排集中写清一次，
后续摄影、表情和材质描写不能改动；禁止无承重悬空、贴合与夹手冲突等矛盾。
指定人数、身份和内容等级要求不变；优先保证支撑与关节、接触可达性，再协调摄影
和自然遮挡，最后考虑变化与装饰。不要求接触面或进入路径全程可见，不为镜头
挪开承重物、强扭身体或减少人物。动态人物与群像消息遵守相同要求。
这些是生成指导，不是物理求解器，也不增加模型评审调用。
...
每帧还需为核心接触保留空间可达的进入路径，保持头颈转向自然，并从完整穿着、前开
