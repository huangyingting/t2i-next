    ...
    for level, refinement in authoring.level_refinements.items():
        for owner in ("shared", "themes", "frames"):
            for index, rule in enumerate(getattr(refinement, owner)):
                yield f"authoring.level_refinements.{level.value}.{owner}[{index}]", rule
...
def story_contract(document: StoryDocument) -> str:
    ...
    paths = [
        root / "_modules" / f"{reference.id}.yaml"
...
def audit_run_configuration(document: StoryDocument) -> StoryRunConfiguration:
    """Explicit audit choices from visual applicability, never a filename map."""
    generation = {}
    if document.requirements.content_levels:
        generation["content_level"] = document.requirements.content_levels[0]
    if document.allocation and document.allocation.type == "fixed_slots":
        path = (
            document._source_path.parent / "_catalogs"
...
@pytest.mark.parametrize(
    "path", sorted((*RECIPES.rglob("*.yaml"), *POLICIES.glob("*.yaml"))),
...
def test_recipe_compilation_selects_only_each_stages_active_level(path):
    ...
    for level in document.requirements.content_levels or tuple(ContentLevel):
        ...
        for stage in StoryStage:
            ...
            excluded = {
                rule
                for other in document.authoring.level_refinements if other != level
                for rule in document.authoring.selected(stage, other)
            } - set(selected)
            assert excluded.isdisjoint(compiled)
            assert resolved.context_for(stage, [resolved.plans[0].theme_id])
...
def test_fixed_couple_catalog_preserves_one_hundred_ordered_plans():
    ...
    configuration = StoryRunConfiguration(generation={
        "theme_count": 100, "frames_per_theme": 1, "output_language": "english",
    })
    ...
    for overrides in (
        InputOverrides(theme_count=99), InputOverrides(female_count=0),
    ...
    changed = resolve_story_input(
        document, InputOverrides(frames_per_theme=2, output_language="chinese"),
...
def test_motion_blur_background_bands_do_not_expand_principal_cast_capacity(level):
    ...
    assert {
        count for band in cast.background_counts
...
@pytest.mark.parametrize(
    "name,product",
    [("film-post", "poster"), ("post-layout", "poster"),
     ("magazine-cover", "magazine_cover"), ("jav-dvd-wrap", "sleeve")],
)
...
def test_visible_copy_is_independent_of_prompt_language_in_both_stages(
    ...
    for stage in StoryStage:
        ...
        assert copy["parameters"] == {
            "product": product, "copy_language": "english", "ascii": "required",
        }
...
def test_bundled_visual_recipe_accepts_external_output_constraints(language, mode):
    ...
    checks = (
        [
            {"type": "word_count", "when_language": "english",
             "min_words": 123, "max_words": 234},
...
def test_confined_pose_and_emotion_cycles_are_not_execution_count_minimums(batch_size):
    ...
    for cast in ({"female_count": 1, "male_count": 3},
...
VISUAL_FACTS = {
    "lifestyle-story": (
        "每日穿搭", "咖啡馆探访、城市漫步、周末旅行", "镜面自拍",
        "定时", "左腿", "右腿", "生活痕迹", "二维码",
    ),
    "intimate-liquid-editorial": (
        "站姿与弓步", "跪姿与蹲姿", "坐姿与椅子", "仰卧与桥式", "侧卧与蜷曲",
        "俯卧", "湿房、浴缸与水面", "家具、台面与建筑支撑", "双人编辑构图",
        "液体起点", "相机视线", "一至三件", "一至两件", "零至四件",
    ),
    "indoor-pure-desire-editorial": (
        "室内", "窗", "床", "沙发", "镜", "姿势", "光线",
    ),
    "piss": (
        "深蹲", "前倾", "蹲便器", "手机", "地砖", "陶瓷", "顶灯",
        "独立身体", "独立地面槽位", "25–34", "65–79",
    ),
    "confined-exhibition-fantasy": (
        "狭小空间", "出口开启", "身体折叠", "抬高骨盆", "对角伸展",
        "跪姿前倾", "侧卧卷曲", "俯卧抬臀", "坐姿折叠", "主表演者", "围观者",
    ),
    "intimate-lifestyle-portrait": (
        "高调", "东亚", "自然", "腮红", "蜜桃", "配饰", "猫眼",
        "毛孔", "绒毛", "全身", "镜头",
    ),
    ...
    "giant-country-fantasy": (
        "访客", "巨人", "尺度", "支撑", "接触", "衣物", "建筑",
    ),
    "furry-mythic-interactions": (
        "兽人", "人类", "两臂两腿", "左右大腿", "膝、小腿、脚",
        "尾巴", "承重", "35–65 毫米", "四分之三身", "全身",
    ),
    "dress": (
        "六个", "视图", "服装", "配饰", "身体", "材质", "版式",
    ),
    "post-layout": (
        "二十世纪中叶", "摄影蒙太奇", "拼贴海报", "主导照片",
        "较小摄影碎片", "标题", "票券", "半色调", "撕边",
    ),
    "film-post": (
        "院线", "片名", "宣传语", "演职员表", "署名", "档案", "排版",
    ),
    "jav-dvd-wrap": (
        "封底", "书脊", "封面", "43", "46", "47", "50", "条码", "13 位",
    ),
    "everyday-social-caricature": (
        "女性", "讽刺", "互动", "夸张", "道具", "社会", "面部", "剪影",
    ),
    "creative": (
        "两到五个", "熟悉物件", "身体", "物件之外", "2 列 3 行",
        "六个", "微缩", "明亮",
    ),
    "edo-warai-e": (
        "江户", "ukiyo-e", "nishiki-e", "平坦", "色块", "轮廓", "三",
        "衣袖", "obi", "屏风", "幽默",
    ),
    "ming-gongbi-mixi-tu": (
        "1573至1644", "江南", "熟绢", "游丝", "铁线", "界画", "三矾九染",
        "五至七个", "散点透视", "茶褐色氧化", "矿物色", "画布",
    ),
    "tang-guohua-figures": (
        "618至907", "绢本重彩", "铁线描", "矿物色平涂", "渲染", "披帛",
        "曲江", "马球场", "齐胸", "半臂", "胡服", "幂篱", "帷帽",
    ),
    "pose": (
        "六种", "姿势", "方位", "俯仰", "身体", "摄影", "关节", "视图",
    ),
    "threshold-emergence": (
        "门", "边界", "身体", "接触", "光", "材质",
    ),
    ...
    "precise-intimate-activity-geometry": (
        "骨盆", "支撑", "轴", "遮挡", "左", "右", "连续", "接触",
    ),
    "surreal-conceptual-portrait": (
        "实体装置", "面无表情", "一个主导隐喻", "负空间", "百分之四十至七十",
        "两种主导色相加一种点缀", "40-105", "承重路径", "亚克力",
    ),
    "demon-lord": (
        "核心恶魔", "王座", "二十五至七十九", "24-135mm",
        "低饱和", "翅膀", "盔甲", "前景", "中景", "背景",
    ),
    "angel": (
        "核心天使", "一对翅膀", "无翼人类", "二十五至七十九",
        "低饱和", "盔甲", "前景", "中景", "背景", "翅膀净空",
    ),
    "motion-blur-photography": (
        "35-50mm", "闪光", "景深", "左", "右", "载体", "拖迹",
        "1/15", "1/4", "相机", "腾空衣物", "手部释放",
    ),
}
...
def test_recipe_preserves_visual_medium_cast_geometry_and_design_facts(name):
    ...
    assert resolve_story_input(
        document, run_configuration=audit_run_configuration(document)
    ).request.story == document.description
...
def test_painting_medium_is_visual_not_a_verbatim_english_output_template(name, level):
    ...
    if name == "edo-warai-e":
        ...
        assert "画布" in frames
    for retired in (
        "逐字保留这五句", "以下列四句原文开头", "空间位置句与当下动作句各须少于60词",
