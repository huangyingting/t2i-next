...
def test_everyday_social_caricature_centers_women_and_lived_interaction() -> None:
    ...
    assert len(brief.splitlines()) <= 250
    assert len(brief) <= 20_000
    assert "two women and one man" not in normalized

    required_contract = (
        "Create original, woman-centered editorial caricatures",
        "EAST ASIAN CAST LOCK",
        "Every visible person is a fictional adult aged twenty-five or older",
        "mainland Chinese",
        "Taiwanese",
        "Hong Kong Chinese",
        "Japanese",
        "South Korean",
        "Singaporean Chinese",
        "Set each Theme in mainland China, Taiwan, Hong Kong, Japan, South Korea",
        "State each person's allowed identity in the Theme premise",
        "Cast quantity comes only from the generation request",
        "Use exactly the requested female count, male count, and total",
        "Never infer, default, demonstrate, or hard-code a cast quantity",
        "Add no bystander, reflection, portrait, mannequin, child",
        "Write every Theme field and complete Frame in English",
        "Use only ASCII U+0020 through U+007E",
        ...
        "This is a deliberately assembled single-poster montage",
        "never people sharing a room or lens perspective",
        "The central woman's action applies a visible force",
        "ACTION-OBJECT COUPLING GATE",
        "one closed mechanical circuit around the same primary object",
        "directional action, exact contact point",
        "The changed object must return force into the next adult",
        "No one performs a separate task",
        "removing one adult's action leaves the object's state",
        "SATIRICAL CARICATURE HARD GATE",
        "selective enlargement or compression",
        "roughly two to four times ordinary visual scale",
        "a giant hand is as wide as its owner's torso",
        "at least one interacting adult with a different, equally obvious deformation",
        "one monumental metaphor occupying thirty-five to fifty percent",
        "exactly two smaller symbols from the same metaphor family",
        "Keep every adult recognizable and anatomically connected",
        "DRAW FIRST, DESCRIBE SECOND",
        "privately compose at least three radically different",
        "WOMAN-CENTERED AGENCY",
        "Nobody merely watches, waits, poses, points, or decorates",
        "At least two adults directly grip, load, brace, block",
        "Choose a physical mechanism with naturally blank surfaces",
        "Both supporting symbols are unmarked physical shapes",
        "Honor the requested content level without letting compliance language dominate",
        "FLAT REAL-PERSON PHOTOMONTAGE",
        "premium physical editorial photomontage",
        "visibly cut, mask, enlarge, compress, rotate, overlap, and remount",
        "Use one integrated poster composition",
        "one orthographic poster plane",
        "ENGLISH IMAGE LABELS",
        "Locked image labels: FIRST LABEL | SECOND LABEL.",
        "The two carriers are the only text-bearing surfaces",
        "Never place labels in a bottom title row",
        "inside the central eighty percent",
        "THEME CONTRACT",
        "FRAME CONTRACT",
        "State the cast declaration once only and never revise its numbers",
        "CONTINUITY AND VARIATION",
        "non-East-Asian identity",
    )
